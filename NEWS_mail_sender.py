...
def send_mail():
    response = messagebox.askyesno("확인 알림","메일을 보내시겠습니까?")
    if response == 1:
        news = get_news()
        msg = EmailMessage()
        msg["Subject"] = ent_subject.get()
        ...
        try:
            ...
            messagebox.showerror("에러","메일을 보내지 못했습니다.")
...
# Mail info frame
mail_frame = LabelFrame(root,text="메일 설정",relief="solid",bd=1)
...
Label(label_frame ,text="수신자").pack(padx=5,pady=5)
Label(label_frame ,text="참조자").pack(padx=5,pady=5)
...
ent_from.insert(0,"yonghwa814@gmail.com")
ent_to = Entry(entry_frame,width=42)
ent_cc = Entry(entry_frame,width=42)
ent_subject.pack(padx=3,pady=5)
ent_from.pack(padx=3,pady=5)
ent_to.pack(padx=3,pady=5)
ent_cc.pack(padx=3,pady=5)
ent_from.config(state="readonly")
